...
class Model:
    ...
    def __init__(self, **kwargs):
        ...
        units = [1,32,32,32,32,32,32,1]

        # print("<<<<<TODO layers>>>>>")
        layer_0_ex = ExchangeableLayer(units=units[0:2], pool_mode=pool_mode, activation=activation, scope='layer_0_ex', reuse=None)
        # layer_1_dr = FeatureDropoutLayer()
        layer_2_ex = ExchangeableLayer(units=units[1:3], pool_mode=pool_mode, activation=activation, scope='layer_2_ex', reuse=None)
        # layer_3_dr = FeatureDropoutLayer()
        layer_4_ex = ExchangeableLayer(units=units[2:4], pool_mode=pool_mode, activation=activation, scope='layer_4_ex', reuse=None)
        layer_6_ex = ExchangeableLayer(units=units[3:5], pool_mode=pool_mode, activation=activation, scope='layer_6_ex', reuse=None)
        layer_8_ex = ExchangeableLayer(units=units[4:6], pool_mode=pool_mode, activation=activation, scope='layer_8_ex', reuse=None)
        layer_10_ex = ExchangeableLayer(units=units[5:7], pool_mode=pool_mode, activation=activation, scope='layer_10_ex', reuse=None)
        layer_12_ex = ExchangeableLayer(units=units[6:8], pool_mode=pool_mode, activation=None, scope='layer_12_ex', reuse=None)

        self.layers = {}
        self.layers['layer_0_ex'] = layer_0_ex
        # self.layers['layer_1_dr'] = layer_1_dr
        self.layers['layer_2_ex'] = layer_2_ex
        # self.layers['layer_3_dr'] = layer_3_dr
        self.layers['layer_4_ex'] = layer_4_ex
        self.layers['layer_6_ex'] = layer_6_ex
        self.layers['layer_8_ex'] = layer_8_ex
        self.layers['layer_10_ex'] = layer_10_ex
        self.layers['layer_12_ex'] = layer_12_ex
    ...
    def get_output(self, team_player, team_match, reuse=None):
        tp, tm = team_player, team_match
        for l, layer in self.layers.items():
            tp, tm = layer.get_output(tp, tm, reuse=reuse)
        return tp, tm
